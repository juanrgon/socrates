...
4. Show only the evidence needed for that inference. For code, render each excerpt with its original line numbers inline, for example via `nl -ba`, and label it with the file path and current line range, such as `pkg/llmrequest/stages/retry.go:253-256`. A path-and-range citation alone is not sufficient: the line numbers must be visible in the excerpt. Keep the excerpt minimal.
...
- Always show code excerpts with their original line numbers inline, for example as `nl -ba` output, in addition to citing the file path and line range. A bare path-and-range citation without visible line numbers is not sufficient.
